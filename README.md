...
GitHub Pages 通过 `.github/workflows/pages.yml` 发布。workflow 在 `master` push、手动触发和每天定时任务时执行：checkout 站点仓库和源文档仓库，安装依赖，运行 `pnpm run sync:docs` 解析 ZIP 并同步文档，再运行 `pnpm run build` 生成 `site/.vitepress/dist` 并上传到 Pages。
