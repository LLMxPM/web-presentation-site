      ...
      <div class="flex-1 flex flex-col justify-center space-y-4">
        ...
        <div class="bg-background-subtle rounded-lg p-4 border border-border-subtle">
          ...
          <div class="space-y-2 text-lg text-secondary">
            <p><Icon name="check" class="size-5 inline-block align-middle" /> 可能<strong class="text-accent2">外推</strong>到训练中未见过的更长序列</p>
            <p><Icon name="check" class="size-5 inline-block align-middle" /> 相对位置偏移可由<strong class="text-primary">线性变换</strong>表示</p>
            <p><Icon name="check" class="size-5 inline-block align-middle" /> 偶数维用 sin，奇数维用 cos；波长从 2π 到 10000·2π</p>
          </div>
        </div>
        ...
        <div class="bg-accent2/10 rounded-lg p-3 border border-accent2/30">
          <p class="text-base text-primary leading-relaxed">
            <Icon name="key" class="size-5 inline-block align-middle" /> 协作逻辑：<strong class="text-accent2">Attention</strong> 做「token 间通信」→ <strong class="text-accent2">FFN</strong> 做「每个 token 内部特征变换」→ <strong class="text-accent2">位置编码</strong> 补回顺序信息。
          </p>
        </div>
      </div>
    </div>
