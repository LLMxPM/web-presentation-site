...
import ThreeLineTable from '@workspace-components/CMP20260709003/v/2'
import Icon from '@runtime-kit/public/components/primitives/Icon.v1.vue'

const mtColumns = [
  { key: 'model', title: '模型', width: '3fr', align: 'left' },
  { key: 'enDe', title: 'EN-DE BLEU', width: '2fr', align: 'center' },
  { key: 'enFr', title: 'EN-FR BLEU', width: '2fr', align: 'center' },
  { key: 'flops', title: '训练 FLOPs', width: '2fr', align: 'center' },
]
...
const ablColumns = [
  { key: 'group', title: '组', width: '1fr', align: 'center' },
  { key: 'variant', title: '变化点', width: '3fr', align: 'left' },
  { key: 'setting', title: '关键设置', width: '4fr', align: 'left' },
  { key: 'ppl', title: 'Dev PPL', width: '1.5fr', align: 'center' },
  { key: 'bleu', title: 'Dev BLEU', width: '1.5fr', align: 'center' },
]
    ...
    <div class="h-full w-full flex gap-5 px-6 pt-1">
      ...
      <div class="flex-1 flex flex-col gap-3 min-w-0">
        ...
        <div class="shrink-0 bg-accent1/10 rounded-lg p-3 border border-accent1/30">
          <p class="text-sm text-primary leading-relaxed">
            <Icon name="trophy" class="size-5 inline-block align-middle" /> <strong class="text-accent1">Transformer big</strong>：EN-DE 28.4 BLEU（超过所有已报道模型）；EN-FR 41.8 BLEU。训练成本（FLOPs）<strong class="text-accent2">显著低于</strong>对比系统。
          </p>
        </div>
      </div>
      ...
      <div class="flex-1 flex flex-col gap-3 min-w-0">
        ...
        <div class="shrink-0 bg-accent2/10 rounded-lg p-3 border border-accent2/30">
          <p class="text-sm text-primary leading-relaxed">
            <Icon name="clipboard" class="size-5 inline-block align-middle" /> <strong class="text-accent2">消融结论</strong>：单头 ↓0.9 BLEU；过多头也变差；更大模型更好；dropout 防过拟合关键；learned PE ≈ sinusoidal PE。4 层 Transformer 在 WSJ parsing 达 F1 91.3（discriminative）/ 92.7（semi-supervised）。
          </p>
        </div>
      </div>
    </div>
